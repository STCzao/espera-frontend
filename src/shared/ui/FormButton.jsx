...
export function FormButton({ children, icon: Icon, isPending, pendingLabel, type = 'submit', variant = 'outline', ...props }) {
  return (
    <button
      className={`inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-lg px-4 font-semibold transition-all focus:outline-none focus:ring-4 focus:ring-espera-purple-soft disabled:cursor-not-allowed disabled:opacity-70 ${variantClasses[variant]}`}
      disabled={isPending}
      type={type}
      {...props}
    >
